...
export default function ReadChapter() {
  ...
  const { data, isLoading, error } = useQuery({
    queryKey: ['chapter', id],
    queryFn: async () => {
      const response = await fetch(`${API_URL}/read?chapterId=dandadan/${id}&provider=mangahere`)
      if (!response.ok) {
        throw new Error('Network response was not ok')
      }
      const data = await response.json()
    ...
    if (isAutoScrolling && data.length && currentImageIndex < data.length) {
      scrollTimer = setInterval(() => {
        ...
        if (imageElements[currentImageIndex]) {
          imageElements[currentImageIndex].scrollIntoView({ behavior: 'smooth' });
  ...
  const hasNextChapter = currentChapterIndex !== undefined && 
    currentChapterIndex > 0

  const hasPreviousChapter = currentChapterIndex !== undefined && 
  ...
  const handleIntervalChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    const value = Math.max(5000, Math.min(60000, parseInt(e.target.value) * 1000));
    setScrollInterval(value);
    localStorage.setItem('scrollInterval', value.toString());
  };

  if (isLoading) {
    return <div className="flex justify-center p-8 items-center bg-gray-900">
  ...
  if (error) {
    return <div className="text-red-500 p-4 bg-gray-900">Error loading chapter</div>
  }

  if (!data?.length) {
    return <div className="p-4 text-white bg-gray-900">No images available</div>
  }
  ...
  return (
    <div className="min-h-screen bg-gray-900 relative">
      <div className="fixed bottom-8 right-8 flex flex-col gap-4 z-50 items-center">
        ...
        <div className="bg-gray-800 p-3 rounded-lg shadow-lg flex flex-col gap-2">
      ...
      <div className="max-w-4xl mx-auto py-8 px-4">
        {chapterList && (
          <h2 className="text-2xl font-bold text-center mb-8 text-white">
            Chapter {chapterList.chapters[currentChapterIndex]?.title.split("Ch.").pop()}
          </h2>
        )}
        <div className="space-y-4">
          {data.map((image: ChapterImage) => (
            <div key={image.page} className="w-full flex justify-center">
              ...
              <img
                src={`/api/fetchImage?imgUrl=${encodeURIComponent(image.img)}&referer=${encodeURIComponent(image.headerForImage.Referer)}`}
                alt={`Page ${image.page}`}
                className="max-w-full h-auto"
                loading="lazy"
                referrerPolicy="no-referrer"
                style={{
                  backgroundImage: `url(/api/fetchImage?imgUrl=${encodeURIComponent(image.img)}&referer=${encodeURIComponent(image.headerForImage.Referer)})`,
